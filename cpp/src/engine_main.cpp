...
static ShmFrameQueue* g_visible_queue = nullptr;
static ShmFrameQueue* g_uv_queue = nullptr;
...
int arcvision_engine_init(const char* visible_engine_path,
    ...
    if (!g_detector->init()) {
        fprintf(stderr, "[Engine] Failed to initialize arc detector\n");
        delete g_detector;
        g_detector = nullptr;
        return -1;
    }
...
void arcvision_engine_set_queues(void* visible_queue, void* uv_queue) {
    g_visible_queue = static_cast<ShmFrameQueue*>(visible_queue);
    g_uv_queue = static_cast<ShmFrameQueue*>(uv_queue);
}
...
int arcvision_engine_process_frame(const unsigned char* visible_gpu,
    ...
    if (!events.empty() && g_publisher) {
        ...
        if (rc != 0) {
            fprintf(stderr, "[Engine] ZMQ publish failed or would block (rc=%d)\n", rc);
        }
    }
...
void arcvision_engine_run_loop(int64_t sync_tolerance_ns) {
    if (!g_visible_queue || !g_uv_queue) {
        fprintf(stderr, "[Engine] Queues not set, cannot run loop\n");
        return;
    }

    fprintf(stderr, "[Engine] Starting detection loop with sync tolerance %lld ns\n",
            static_cast<long long>(sync_tolerance_ns));
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        int rc_vis = shm_queue_pop(g_visible_queue, &vis_header, vis_data);
        if (rc_vis != 0) {
            std::this_thread::sleep_for(std::chrono::microseconds(500));
            continue;
        }

        int rc_uv = shm_queue_pop(g_uv_queue, &uv_header, uv_data);
        if (rc_uv != 0) {
            std::this_thread::sleep_for(std::chrono::microseconds(500));
            continue;
        }
        ...
        if (pts_diff > sync_tolerance_ns) {
            fprintf(stderr, "[Engine] Frame sync mismatch: %lld ns\n",
                    static_cast<long long>(pts_diff));
            continue;
        }
    ...
    delete[] vis_data;
    delete[] uv_data;
    fprintf(stderr, "[Engine] Detection loop stopped\n");
}
